...
![image.png](https://raw.githubusercontent.com/kyriekevin/img-hosting/main/imgs20260125141021115.png)
...
* **On-Policy**：给定输入prompts，从学生模型自身采样来生成序列（rollout），并为这些序列分配奖励（reward）。
  ...
  * 常用方式二：
        ![image.png](https://raw.githubusercontent.com/kyriekevin/img-hosting/main/imgs20260125143914090.png)
...
* **Author's Hypothesis:** Experience with Guidance
  ...
  * The "Reverse KL" Intuition: Mode-Seeking vs. Mean-Seeking
    ...
    * **SFT (Forward KL $D_{KL}(P || Q)$)**: 倾向于 Zero-Avoiding。为了最小化 Forward KL，只要 Teacher 分布 $P$ 的地方，Student 分布 $Q$ 就必须。如果 Teacher 的分布具有多样性（例如，对一个问题有 A、B、C 三种正确的解法），SFT 训练出来的 Student 往往会尝试“平均”这三种解法，结果生成出一种语法通顺但逻辑混乱的混合物（Mean-Seeking）。
...
* **Core Architecture:**
  ...
  * Case Study
    ![image.png](https://raw.githubusercontent.com/kyriekevin/img-hosting/main/imgs20260125162516073.png)
